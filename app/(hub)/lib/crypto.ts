...
const JWT_SECRET = process.env.JWT_SECRET || 'dev_secret_change_me_in_production_9d72ba'
 ...
 */
export async function createCorrelationId(existingId?: string): Promise<string> {
  if (existingId && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(existingId)) {
    return existingId
  }
  return uuidv4()
}
 ...
 */
export async function createUploadProof(payload: UploadProofPayload): Promise<string> {
 ...
 */
export async function verifyUploadProof(token: string): Promise<UploadProofPayload | null> {
  try {
